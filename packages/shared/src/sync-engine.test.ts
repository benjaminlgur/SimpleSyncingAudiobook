...
const position = (positionMs: number, updatedAt: number): PlaybackPosition => ({
  audiobookId: "book",
  chapterIndex: 0,
  positionMs,
  updatedAt,
...
function create(
  push = vi.fn(
    async (_position: PlaybackPosition): Promise<SyncPushResult> => accepted,
  ),
) {
...
test("offline progress wins over an older server position regardless of initialization order", async () => {
  ...
  expect(engine.reconcilePosition(position(1000, 100))).toMatchObject(
    position(9000, 200),
  );
});
...
test("a successful pause and close retain the offline resume position", async () => {
  ...
  expect(await create().initialize()).toMatchObject({
    chapterIndex: 3,
    positionMs: 90000,
  });
});
...
test("a rejected stale request cannot overwrite a seek made while it was in flight", async () => {
  ...
  await first;
  expect(engine.getState().pending).toMatchObject({
    chapterIndex: 4,
    positionMs: 8000,
  });
});
...
test("offline forks survive restart and require an explicit choice", async () => {
  const engine = create(
    vi.fn(async (): Promise<SyncPushResult> => ({
      accepted: false,
      serverPosition: { ...position(2000, 1), revision: 2 },
    })),
  );
  ...
  expect(restored.getState()).toMatchObject({
    pending: { positionMs: 8000 },
    conflict: { positionMs: 2000, revision: 2 },
  });
  await restored.manualSync();
  ...
  expect(restored.getState()).toMatchObject({
    pending: { positionMs: 8000, revision: 3, dirty: false },
    conflict: null,
  });
});
...
test("scrubbing saves locally immediately and coalesces remote writes", async () => {
  vi.useFakeTimers();
  const push = vi.fn(async () => accepted);
  const engine = create(push);
  for (const ms of [1000, 3000, 10000]) {
    engine.updatePosition(0, ms);
    await engine.onSeek();
  ...
  expect(push).toHaveBeenLastCalledWith(
    expect.objectContaining({ positionMs: 10000 }),
  );
  vi.useRealTimers();
});
