...
test("hashes bytes with bounded reads and detects incomplete imports", async () => {
  ...
  expect(await fingerprintFile(3, async () => abc)).toBe(
    "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad",
  );
  await expect(fingerprintFile(4, async () => abc)).rejects.toThrow(
    "File changed",
  );
  const sizes: number[] = [];
  await fingerprintFile(3 * 1024 * 1024 + 2, async (_, length) => {
    sizes.push(length);
