...
export function recordingFingerprint(fileHashes: string[]): string {
  if (!fileHashes.length || fileHashes.some((h) => !/^[a-f0-9]{64}$/.test(h)))
    throw new Error("Invalid SHA-256 file fingerprints");
...
export async function fingerprintFile(
  ...
  for (let offset = 0; offset < size;) {
    ...
    if (chunk.length !== length)
      throw new Error("File changed or could not be read during import");
    hash.update(chunk);
    offset += length;
  }
