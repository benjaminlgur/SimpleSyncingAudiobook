...
function isSingleFile(chapters: ChapterInfo[]) {
  return (
    chapters.length > 0 &&
    chapters.every((chapter) => chapter.filename === chapters[0].filename)
  );
}
...
export function toSyncPosition(
  ...
  return {
    chapterIndex: 0,
    positionMs:
      (chapters[position.chapterIndex]?.startMs ?? 0) + position.positionMs,
  };
}

export function fromSyncPosition(
  chapters: ChapterInfo[],
  position: Position,
): Position {
  if (!isSingleFile(chapters)) return position;
  ...
  return {
    ...
    positionMs: Math.max(
      0,
      absoluteMs - (chapters[chapterIndex]?.startMs ?? 0),
    ),
  };
}
