...
const mutation = makeFunctionReference<"mutation", { book: string }>(
  "positions:update",
);
let backgroundPush: ReturnType<typeof useCloudMutation<typeof mutation>>;
function Screen() {
  useCloudQuery(query, { book: "book" });
  backgroundPush = useCloudMutation(mutation);
  return null;
}
const app = (ready: boolean, visible = true) =>
  createElement(CloudProvider, {
    ready,
    children: visible ? createElement(Screen) : null,
  });

test("cached local access never subscribes or queues writes before cloud authorization", async () => {
  await act(async () => {
    renderer = create(app(false));
  });
...
test("a background service retains live access after its screen closes, but loses it on logout", async () => {
  ...
  await act(async () => {
    renderer.update(app(true, false));
  });
