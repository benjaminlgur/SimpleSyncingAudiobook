...
export class AudioTokenizer extends AbstractTokenizer {
  ...
  async peekBuffer(buffer: Uint8Array, options?: IReadChunkOptions) {
    ...
    const length = Math.min(
      opts.length,
      Math.max(0, this.fileInfo.size - opts.position),
    );
    if (
      length > buffer.length ||
      length < 0 ||
      opts.position < 0 ||
      this.bytesRead + length > this.budget
    )
      throw new Error("Audio metadata exceeds read budget");
    let read = 0;
    while (read < length) {
      const data = await invoke<number[]>("read_audio_range", {
        path: this.path,
        offset: opts.position + read,
        length: Math.min(1024 * 1024, length - read),
      });
