...
vi.mock("../hooks/useAudioPlayer", () => ({
  useAudioPlayer: (options: typeof mocks.audioOptions) => {
    ...
    return [
      {
        currentChapterIndex: options.initialChapterIndex,
        positionMs: options.initialPositionMs,
        durationMs: 10000,
        playbackSpeed: 1,
      },
      {},
...
test("startup waits for the saved offline position even when the remote query is already cached", async () => {
  ...
  vi.stubGlobal(
    "window",
    Object.assign(new EventTarget(), { navigator: { onLine: true } }),
  );
  vi.stubGlobal(
    "document",
  ...
  await act(async () => {
    renderer = create(
      createElement(Player, {
        ...
        storageScope: "scope",
        onBack: vi.fn(),
        onConvexIdResolved: vi.fn(),
        onRelocate: vi.fn(),
      }),
    );
  });
  expect(mocks.audioOptions).toMatchObject({
    initialChapterIndex: 1,
    initialPositionMs: 9000,
  });
});
