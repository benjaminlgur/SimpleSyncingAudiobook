...
export async function importAudio(
  ...
  try {
    for (const [index, file] of files.entries()) {
      const extension =
        file.name
          .split(".")
          .pop()
          ?.replace(/[^a-zA-Z0-9]/g, "") || "audio";
      const uri = `${directory}${index}.${extension}`;
      await FileSystem.copyAsync({ from: file.uri, to: uri });
      const info = await FileSystem.getInfoAsync(uri);
      if (!info.exists || info.isDirectory)
        throw new Error("Unable to import audio");
      hashes.push(
        await fingerprintFile(info.size, async (position, length) =>
          toByteArray(
    ...
    return {
      files: imported,
      checksum: recordingFingerprint(hashes),
      discard: () => FileSystem.deleteAsync(directory, { idempotent: true }),
    };
  } catch (error) {
